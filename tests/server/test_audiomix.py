...
class AudioMixTests(unittest.TestCase):
    ...
    def make_audio_source(self, channel="source.audio"):
        ...
        self.loop.create_task(self.bus.post(messages.AudioSourceAdded(channel, "127.0.0.1")))
    ...
    def test_add_remove_sources(self):
        ...
        async def consumer(queue):
            while True:
                ...
                queue.task_done()
        self.bus.add_consumer(messages.AudioMixStatus, consumer)
        ...
        future = self.loop.create_future()
        self.loop.create_task(self.bus.post(messages.AudioSourceRemoved("source.audio", "127.0.0.1")))
        self.loop.run_until_complete(future)
        message = future.result()
        self.assertEqual(message.active_source, None)
    ...
    def test_set_audio_source(self):
        ...
        future = self.loop.create_future()
        self.loop.create_task(self.bus.post(messages.SetAudioSource("source.audio1")))
        self.loop.run_until_complete(future)
        message = future.result()
        ...
        future = self.loop.create_future()
        self.loop.create_task(self.bus.post(messages.SetAudioSource("source.audio2")))
        self.loop.run_until_complete(future)
        message = future.result()
        ...
        future = self.loop.create_future()
        self.loop.create_task(self.bus.post(messages.SetAudioSource(None)))
        self.loop.run_until_complete(future)
        message = future.result()
        self.assertEqual(message.active_source, None)
        self.assertEqual(self.amix._sources["source.audio1"].mute, True)
